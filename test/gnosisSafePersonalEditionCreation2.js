...
const Proxy = artifacts.require("./Proxy2.sol")
const ProxyFactory = artifacts.require("./ProxyFactory2.sol")
const MockContract = artifacts.require('./MockContract.sol');
const MockToken = artifacts.require('./Token.sol');

contract('GnosisSafePersonalEdition via ProxyFactory2', function(accounts) {
    ...
    let getCreationData = async function(gasToken, userCosts, creationNonce) {
        ...
        let constructorData = abi.rawEncode(
            ...
            [ gnosisSafeMasterCopy.address, gasToken, userCosts ]
        ).toString('hex')
    ...
    let deployWithCreationData = async function(creationData) {
        let tx = proxyFactory.createProxy(
            gnosisSafeMasterCopy.address, creationData.data, creationData.gasToken, creationData.userCosts, creationData.creationNonce, 
            {from: funder, gasPrice: gasPrice}
        )

        console.log("    Deployed safe address: " + utils.getParamFromTxEvent(
            await tx,
            'ProxyCreated', 'proxy', proxyFactory.address, GnosisSafe, 'create Gnosis Safe',
        ).address)
    }
    ...
    it('should create safe from random account and pay in ETH', async () => {
        ...
        let estimate = (await proxyFactory.createProxy.estimateGas(gnosisSafeMasterCopy.address, gnosisSafeData, 0, 0, creationNonce)) + 9000
    ...
    it('should fail if token payment fails', async () => {
        ...
        // We just set an fix amount of tokens to pay
        let creationData = await getCreationData(token.address, 1337, new Date().getTime())
        await deployWithCreationData(creationData)
        assert.equal(await web3.eth.getCode(creationData.safe), '0x')
        ...
        creationData = await getCreationData(mockToken.address, 1337, new Date().getTime())
        await deployWithCreationData(creationData);
        assert.equal(await web3.eth.getCode(creationData.safe), '0x');
